...
{
	//recursion end condition -- hit the end T
	if (k == no_of_divisions) 
	{
		if (initial_stock_price*pow(up_factor, ((float) i)) > barrier_price) 
		{
			// the probability that the path that the stock price took, starting from initial_stock_price
			// and eventually getting to (initial_stock_price)*up_factor^i has hit the barrier_price on its 
			// way is given by the expression derived by Baldi, Caramellino and Iovino in their 1999 paper
			// (Math Finance, Vol. 9, pages 293-322)... Barone-Adesi, Fusari and Theal suggest "adjusting"
			// the uptick_prob and downtick_prob by (1-prob that the stock path has crossed the barrier) on
			// its journey in the binomial tree model.  
			...
			return ( (1-probability_stock_path_has_hit_barrier_by_the_time_it_got_here)*
					max(0.0, (initial_stock_price*pow(up_factor, ((float) i))) - strike_price) );
		}
		else
			return (0.0);
	}
	...
	if (k == no_of_divisions) 
	{
		if (initial_stock_price*pow(up_factor, ((float) i)) > barrier_price) 
		{
			...
			exp( (-2.0/(volatility*volatility*expiration_time)) *
				...
				log(initial_stock_price*pow(up_factor, ((float) i))/barrier_price) );			
			return ( (1-probability_stock_path_has_hit_barrier_by_the_time_it_got_here)*
					max(0.0, (strike_price -initial_stock_price*pow(up_factor, ((float) i)))) );
		}
		else
			return (0.0);
	}
...
int main (int argc, char* argv[])
{
	...
	cout << "Binomial Lattice Models with Baron-Adesi, Fusari and Theal Adjustment" << endl;
	...
	cout << "--------------------------------------" << endl;
	cout << "Price of an European Down and Out Call Option = " << european_down_and_out_call_option(0, 0) << endl;
	cout << "Price of an European Down and Out Put Option = " << european_down_and_out_put_option(0, 0) << endl;
	cout << "--------------------------------------" << endl;
