...
//  prog4
//
//  Created by Jay Chen on 9/19/19.
...
int Rank(Matrix A)
{
    ...
    int no_of_rows_of_A = A.nrows();
    int no_of_cols_of_A = A.ncols();
...
void create_solution_vector(Matrix A, ColumnVector y)
{
    ...
    
    {
        ...
        //If the function can determine the previous permutation, it rearranges the elements as such and returns true.
        ...
        do
        {
            ...
            for (int i = 0; i < A.ncols(); ++i)
            {
                ...
                //if i pick col(i), then put this col into the B
               
                if( did_i_pick_this_col[i])
                {
                   //add A.colmn(i) in to B
//                    cout<<" i pick"<< i <<"columne"<<endl;
                    vector<double> temp;
                    for(int a=0; a< A.nrows(); a++)
                    {
                        temp.push_back(A(a+1,i+1));
                    }
                    B.push_back(temp);
                }
            }
            ...
            //cout<<" column reduced version of A"<<endl;
//            cout << setw(10) << Column_reduced_version_of_A  << endl;
            
            //find one with rank==rank(A), this is the one we need to solve the basic solutions
...
//                cout<< "rank of a "<< rank_of_A<<endl;
//                cout<<"soln "<<endl;
//                cout << setw(10) << soln  << endl;
                ...
                //initialize a vector temp1 all 0, and find the columns which we just picked, add it into the temp
...
//    cout<<"solu size: "<<solution_set.size()<<endl;
    //the solution was put row by row in solution_set, so need to transpose it.
    for (int j = 1; j <= A.ncols(); j++)
        for (int k = 1; k <= solution_set.size(); k++)
            solution(j,k) = solution_set[k-1][j-1];
//    cout<< "solution set "<<solution.nrows()<<" "<< solution.ncols()<< endl;
//    cout << setw(10) << solution << endl;
    
    int rank_of_solution_set = Rank(solution);
//    cout<<"rank of solution：  "<<Rank(solution)<<endl;
    
    {
        ...
        do
        {
           ...
           // cout<<"TEST************1"<<endl;
            ...
            // of the local-matrix "solution" and stores these columns in C
            //
            // for this part, I once made a terrible mistake. because,in the first input file, A.nrows == A.ncols, in the for loop, it's totally fine for me to use solution.ncols()  to replace solution.rows, and it just works, gives out the right solution. However, i got mistakes in input2&4. In the input4, A.ncols>A.nrows, so the for loop doesn't work,  turns out the int a is out of range, so i figured out why (a) is out of range, i mean, it sure does. So i use solution.ncols() - rank_of_solution_set +1 instead of  solution.ncols, it works again. But not for input2.
            //And finally, when I'm goning to the input2, the same range problem. Suddenly, I just realized, I can use solutions.nrows. That made sence.
            //so i know how to solve it correctly. just want to leave the code this way.
            //i mean the if(), else if(), else(), this three part are the same. they are equivalent to the third situlation.
            //
            if(A.ncols() > A.nrows())
            {
                for (int i = 0; i < solution.ncols(); ++i)
                {
                    ...
                    {
                        //add solution.colmn(i) in to C
                        vector<double> temp;
                        for(int a = 0; a < solution.ncols() - rank_of_solution_set +1 ; ++a)
                        {
                            temp.push_back( solution(a+1,i+1) );
//                            cout<<temp[a]<<" ";
                        }
//                        cout<<endl;
                        C.push_back(temp);
            ...
            else if( A.ncols() == A.nrows())
            {
                for (int i = 0; i < solution.ncols(); ++i)
                {
                    if( did_i_pick_this[i] )
                    {
                        //add solution.colmn(i) in to C
                        vector<double> temp;
                        for(int a = 0; a < solution.ncols()  ; ++a)
                        {
                            temp.push_back( solution(a+1,i+1) );
//                            cout<<temp[a]<<" ";
                        }
            ...
            else
            {
                for (int i = 0; i < solution.ncols(); ++i)
                {
                    if( did_i_pick_this[i] )
                    {
                        //add solution.colmn(i) in to C
                        vector<double> temp;
                        for(int a = 0; a < solution.nrows() ; ++a)
                        {
                            temp.push_back( solution(a+1,i+1) );
//                            cout<<temp[a]<<" ";
                        }
...
//            cout<<"column reduced version"<<endl;
            ...
            for (int j = 1; j <= solution.nrows(); j++)
                ...
                {
                    Column_reduced_version(j,k) = C[k-1][j-1];
                }
...
int main (int argc, char* argv[])
{
    ...
    A(1,1) = 16.0;  A(1,2) = 2.0;  A(1,3) = 6.4;
    A(2,1) = 2.0;   A(2,2) = 1.0;  A(2,3) = 3.2;
    A(3,1) = 6.4;   A(3,2) = 3.2;  A(3,3) = 64.0;
    
    cout << setw(9) << A << endl;
    
    LowerTriangularMatrix L = Cholesky(A);
